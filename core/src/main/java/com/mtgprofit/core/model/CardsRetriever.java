    ...
    public List<CardsProfit> getAllCards(List<Expansion> expansions, List<Shop> shops, BigDecimal minimalPrice) {
        ...
        for(Shop shop : shops){
            for(String cardName : cards.get(shop).keySet()){
                for(Shop s : shops){
                    if(shop!=s&&cards.get(s).get(cardName)!=null&&cards.get(s).get(cardName).getBuyPrice()!=null&&cards.get(s).get(cardName).getSellPrice()!=null){
                        ...
                        if(profit.doubleValue()>0){
                            cardsProfits.add(new CardsProfit(profit,cardName,
